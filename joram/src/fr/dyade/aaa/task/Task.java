...
public abstract class Task extends fr.dyade.aaa.scbean.AbstractSCBean {
  ...
  public static final String RCS_VERSION="@(#)$Id: Task.java,v 1.1 2002-03-06 16:52:20 joram Exp $"; 
  ...
  private RoleMultiple statusListeners;
  ...
  public Task(short to, String name, AgentId parent, ConditionHandle[] conditions) {
    ...
    statusListeners = new RoleMultiple();
  }
  ...
  protected void setStatus(int status) throws Exception {
    ...
    sendTo(statusListeners, newStatus);
   ...
   * @param parent	agent to report status to
   */
  protected void setParent(AgentId parent) throws Exception {
    if (parent == null)
   ...
   */
  public void addStatusListener(AgentId listener) {
    statusListeners.addListener(listener);
  }
  ...
  public boolean isStatusListener(AgentId listener) {
    if (statusListeners == null)
      return false;
    Enumeration list = statusListeners.getListeners();
    if (list == null)
      return false;
    while (list.hasMoreElements())
      if (listener.equals((AgentId) list.nextElement()))
	return true;
    return false;
  }
