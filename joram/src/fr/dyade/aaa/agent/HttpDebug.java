...
//     if (Debug.debug && (dmon != null)) dmon.stop();
    httpd = null;
  }
  ...
  private HttpDebug(int port) throws IOException {
    ...
    monitors[0] = new HttpDebugMonitor("HttpDebug");

//     if (Debug.debug) dmon = new DebugMonitor();
  }
...
//       System.out.println("request=" + request);
    ...
    void listAgents(StringBuffer buf) {
      ...
      buf.append("<PRE>\n");
      buf.append("now=" + Agent.now + "\n");
      buf.append("NumberAgents=" + Agent.agents.size() + "\n");
      buf.append("NbMaxAgents=" + Agent.NbMaxAgents + "\n");
      buf.append("nbFixedAgents=" + Agent.nbFixedAgents + "\n");
      buf.append("</PRE>\n");
      buf.append("<TABLE BORDER=1 CELLSPACING=0 CELLPADDING=0 WIDTH=\"100%\">\n");
      AgentId list[] = Agent.getLoadedAgentIdlist();
      for (int i=0; i<list.length; i++) {
	Agent agent = (Agent) Agent.agents.get(list[i]);
	buf.append("<TR>\n");
	...
	if (agent != null) {
	  ...
	  buf.append("monitored=" + agent.monitored + "\n");
    ...
    void listServers(String cmd, StringBuffer buf) {
      ...
      buf.append("<TABLE BORDER=1 CELLSPACING=0 CELLPADDING=0 WIDTH=\"100%\">\n");
      for (int i=0; i<AgentServer.servers.length; i++ ) {
	int port = -1;
	ServerDesc desc = AgentServer.servers[i];
	buf.append("<TR>\n");
	...
	sub = 0;
	msgConsId = getArg(cmd.substring(CMD_QUEUE.length()));
      } else if (cmd.startsWith(CMD_START)) {
	sub = 1;
	msgConsId = getArg(cmd.substring(CMD_START.length()));
      } else if (cmd.startsWith(CMD_STOP)) {
	sub = 2;
	msgConsId = getArg(cmd.substring(CMD_STOP.length()));
      }
	...
	buf.append("<TD CLASS=FONT1 ALIGN=\"CENTER\" BGCOLOR=\"#3366FF\"><A href=\"")
	  .append(base).append(CMD_MSG_CONS).append(CMD_STOP + "+").append(i)
	  .append("\">STOP</A>\n")
	  .append("</TD>");

	buf.append("<TD CLASS=FONT1 ALIGN=\"CENTER\" BGCOLOR=\"#3366FF\"><A href=\"")
	  .append(base).append(CMD_MSG_CONS).append(CMD_START + "+").append(i)
	  .append("\">START</A>\n")
	  .append("</TD>");

	buf.append("<TD CLASS=FONT1 ALIGN=\"CENTER\" BGCOLOR=\"#3366FF\"><A href=\"")
	  .append(base).append(CMD_MSG_CONS).append(CMD_QUEUE + "+").append(i)
	  .append("\">QUEUE</A>\n")
	  .append("</TD>");
	...
	if ((msgConsId == i) && (sub == 0) && (cons.getQueue().size() > 0)) {
	  buf.append("<TR><TD COLSPAN=\"3\"><PRE WIDTH=\"80\">");
...
// 	buf.append("</CODE></PRE></TD></TR>\n");
	...
	return;
      ...
      Agent agent = (Agent) Agent.agents.get(id);
      if (agent == null) {
