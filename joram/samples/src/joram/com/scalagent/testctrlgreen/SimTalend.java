...
import com.scalagent.ctrlgreen.ClientFramework;
...
public class SimTalend implements ActionHandler {
  ...
  public static void main(String[] args) throws Exception {
    ...
    timer.schedule(new TimerTask() {
      public void run() {
        ...
        try {
          framework.publishParameters(new MyParameters(parameters));
  ...
  public ActionReturn onAction(Action action) {
    ...
    if (action.getType() == Action.GetInventoryVMware) {
      try {
        String inventory = "<inventory name=\"Inventory\">\n" +
            "  <vm name=\"vm1\"\\>\n" +
            "  <vm name=\"vm2\"\\>\n" +
            "<\\inventory>\n";
        framework.publishInventoryVMWare(new MyInventory(inventory));
