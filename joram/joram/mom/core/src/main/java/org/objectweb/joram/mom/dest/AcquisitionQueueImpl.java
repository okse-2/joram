...
/**
 * The {@link AcquisitionQueueImpl} class implements the MOM acquisition queue
 * behavior, basically acquiring messages periodically or on client request,
 * using an {@link AcquisitionModule}.
 */
public class AcquisitionQueueImpl extends QueueImpl implements AcquisitionQueueImplMBean {

  /** define serialVersionUID for interoperability */
  private static final long serialVersionUID = 1L;

  public static Logger logger = Debug.getLogger(AcquisitionQueueImpl.class.getName());
  ...
  /**
   * Constructs an {@link AcquisitionQueueImpl} instance.
   * 
   * @param adminId
  ...
  public AcquisitionQueueImpl(AgentId adminId, Properties properties) throws RequestException {
    ...
    try {
      ...
      logger.log(BasicLevel.ERROR, "AcquisitionQueueImpl: error with acquisition class.", exc);
  ...
  public ClientMessages preProcess(AgentId from, ClientMessages cm) {
    if (logger.isLoggable(BasicLevel.DEBUG)) {
      logger.log(BasicLevel.DEBUG, "AcquisitionQueueImpl. preProcess(" + from + ", " + cm + ')');
    }
  ...
  public String toString() {
    return "AcquisitionQueueImpl:" + getId().toString();
  }
