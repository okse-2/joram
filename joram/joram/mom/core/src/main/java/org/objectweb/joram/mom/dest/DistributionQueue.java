...
/**
 * An {@link DistributionQueue} agent is an agent hosting a
 * {@link DistributionQueueImpl}.
 */
public class DistributionQueue extends Queue {
  ...
  /**
   * Creates the {@link DistributionQueueImpl}.
   *
  ...
  public DestinationImpl createsImpl(AgentId adminId, Properties prop) throws RequestException {
    return new DistributionQueueImpl(adminId, prop);
  }
  
  public void agentFinalize(boolean lastTime) {
    super.agentFinalize(lastTime);
    ((DistributionQueueImpl) destImpl).close();
  }
