...
import com.nortel.oam.test3.common.Data;
import com.nortel.oam.test3.common.ProgressSetter;
...
public class Producer {
  ...
  private void start() throws Exception {
    ...
    props_2.put("Prop1", "value1");
    props_2.put("Prop2", "value2");
    startMessagesProduction(props_2, "'Producer 2' (right one)", null, triggers[1]);

    // Producer 3
    Hashtable props_3 = new Hashtable();
    props_3.put("Prop1", "value1");
    props_3.put("Prop2", "value2");
    startMessagesProduction(props_3, "'Producer 3' (right one)", null, triggers[2]);
...
//    startMessagesProduction(props_4, "'Producer 4'", null, triggers[3]);
  ...
  private void createQueueListener() throws Exception {
    ...
    final QueueSession requestQueueSession = requestQueueConnection.createQueueSession(false, Session.AUTO_ACKNOWLEDGE);
    ...
    requestQueueConnection.setExceptionListener(new ExceptionListener() {
      public void onException(JMSException e) {
        e.printStackTrace(System.out);
      }
    });
  ...
  private void startMessagesProduction(final Hashtable properties, final String id,
                                       final ProgressSetter progressSetter, final Trigger trigger) {
  ...
  class ProgressToGauge implements ProgressSetter {
    ...
    public ProgressToGauge() {
      gauge = new Gauge("Producer Gauge", closeLock, Props.msgs_count_per_cycle);
