...
public class QueueSession extends fr.dyade.aaa.joram.Session implements javax.jms.QueueSession { 
  ...
  public QueueSession(boolean transacted, int acknowledgeMode, long sessionIDNew, fr.dyade.aaa.joram.Connection refConnectionNew) {
  ...
  protected Vector preparesTransactedAck(long messageJMSMOMID) throws javax.jms.JMSException {
    ...
    while(i>=0) {
      /* add the elment in the ack Vector & removes from the other */
      currentMsg = (fr.dyade.aaa.mom.MessageQueueDeliverMOMExtern) XAMessageToAckVector.elementAt(i);
      currentQueue = (fr.dyade.aaa.mom.QueueNaming) currentMsg.message.getJMSDestination();
      message = currentMsg.message;
      resultVector.addElement(new fr.dyade.aaa.mom.AckQueueMessageMOMExtern(messageJMSMOMID, currentQueue, message.getJMSMessageID(), acknowledgeMode, new Long(sessionID).toString()));
      ...
      int j = i-1;
      while(j>=0) {
	previousMsg = (fr.dyade.aaa.mom.MessageQueueDeliverMOMExtern) XAMessageToAckVector.elementAt(j);
	previousQueue = (fr.dyade.aaa.mom.QueueNaming) previousMsg.message.getJMSDestination();
  ...
  protected void rollbackXAMessage() throws javax.jms.JMSException {
    try {
      ...
      fr.dyade.aaa.mom.QueueNaming[] tabQueue = this.preparesRollback();
	...
	fr.dyade.aaa.mom.XARollbackQueueMOMExtern msgRoll = new fr.dyade.aaa.mom.XARollbackQueueMOMExtern(messageJMSMOMID, new Long(sessionID).toString(), tabQueue);
	/* synchronization because it could arrive that the notify was
	 * called before the wait 
	 */
	synchronized(obj) {
	  refConnection.waitThreadTable.put(longMsgID,new ResponseAckObject(obj, tabQueue.length));
	  /* get the messageJMSMOM identifier */
  ...
  protected void rollbackDeliveryMsg() throws javax.jms.JMSException {
	...
	fr.dyade.aaa.mom.MessageQueueDeliverMOMExtern currentMsg = (fr.dyade.aaa.mom.MessageQueueDeliverMOMExtern) XAMessageToAckVector.remove(XAMessageToAckVector.size()-1);
	...
	refConnection.decreaseMessageMOMID();
