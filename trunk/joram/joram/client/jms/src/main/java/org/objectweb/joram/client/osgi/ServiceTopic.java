 ...
 *
 */
public class ServiceTopic extends ServiceDestination implements ManagedServiceFactory {
  public static final Logger logmon = Debug.getLogger(ServiceTopic.class.getName());
  public static final String PID_NAME = "joram.topic";

  private ServiceRegistration registration;
  private String className = Topic.TOPIC;

  public ServiceTopic(final BundleContext bundleContext) {
    super(bundleContext);
    if (logmon.isLoggable(BasicLevel.DEBUG))
      logmon.log(BasicLevel.DEBUG, "ServiceTopic(" + bundleContext + ')');
  ...
  public void updated(final String pid, final Dictionary properties)
    ...
    if (logmon.isLoggable(BasicLevel.DEBUG))
      logmon.log(BasicLevel.DEBUG, "ServiceTopic.updated(" + pid + ", " + properties + ')');
    ...
    try {
      ...
      new Thread(new Runnable() {
        public void run() {
          doUpdated(pid, properties, className, false);
        }
      }).start();

    } catch (Exception e) {
      if (logmon.isLoggable(BasicLevel.ERROR))
        logmon.log(BasicLevel.ERROR, "ServiceTopic.updated ", e);
  ...
  public void deleted(String pid) {
    if (logmon.isLoggable(BasicLevel.DEBUG))
      logmon.log(BasicLevel.DEBUG, "ServiceTopic.deleted(" + pid + ')');
    super.deleted(pid);
  }
}