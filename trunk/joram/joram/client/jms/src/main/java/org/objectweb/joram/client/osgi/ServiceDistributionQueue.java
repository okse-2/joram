 ...
 *     &lt;property name="jndiName"&gt;DisQueue&lt;/property&gt;
...
public class ServiceDistributionQueue extends ServiceDestination implements ManagedServiceFactory {
  ...
  private String className = Queue.DISTRIBUTION_QUEUE;
  
  private Dictionary properties = null;

  public ServiceDistributionQueue(final BundleContext bundleContext) {
    super(bundleContext);
  ...
  throws ConfigurationException {
  	if (logmon.isLoggable(BasicLevel.DEBUG))
  		logmon.log(BasicLevel.DEBUG, "ServiceDistributionQueue.updated(" + pid + ", " + props + ')');
  	...
  	try {
  		...
  		String dc = (String) properties.get("distribution.className");
  		 if (dc == null)
  		   properties.put("distribution.className", JMSDistributionQueue.JMSDistribution);
	...
	public void deleted(String pid) {
		if (logmon.isLoggable(BasicLevel.DEBUG))
      logmon.log(BasicLevel.DEBUG, "ServiceDistributionQueue.deleted(" + pid + ')');
		super.deleted(pid);
  }
}