...
public class Requestor 
  ...
  private void setStatus(int status) {
    if (JoramTracing.dbgClient.isLoggable(BasicLevel.DEBUG))
      JoramTracing.dbgClient.log(
        BasicLevel.DEBUG, "Requestor.setStatus(" + 
  ...
  public synchronized AbstractJmsReply request(
    ...
    if (JoramTracing.dbgClient.isLoggable(BasicLevel.DEBUG))
      JoramTracing.dbgClient.log(
        BasicLevel.DEBUG, "Requestor.request(" + 
        request + ',' + timeout + ')');
    ...
    if (JoramTracing.dbgClient.isLoggable(BasicLevel.DEBUG))
      JoramTracing.dbgClient.log(
        BasicLevel.DEBUG, " -> request #" + requestId + " wait");
    ...
    if (JoramTracing.dbgClient.isLoggable(BasicLevel.DEBUG))
      JoramTracing.dbgClient.log(
        BasicLevel.DEBUG, " -> request #" + requestId + " awake");
    ...
    try {
      if (status == Status.RUN) {
        ...
        if (JoramTracing.dbgClient.isLoggable(BasicLevel.DEBUG))
          JoramTracing.dbgClient.log(
            BasicLevel.DEBUG, " -> request #" + requestId + " done");      
  ...
  public synchronized boolean replyReceived(AbstractJmsReply reply) 
    ...
    if (JoramTracing.dbgClient.isLoggable(BasicLevel.DEBUG))
      JoramTracing.dbgClient.log(
        BasicLevel.DEBUG, "Requestor.replyReceived(" + 
        reply + ')');
  ...
  public synchronized void errorReceived(
    int replyId, JMSException exc) {
    if (JoramTracing.dbgClient.isLoggable(BasicLevel.DEBUG))
      JoramTracing.dbgClient.log(
        BasicLevel.DEBUG, "Requestor.errorReceived(" + 
        replyId + ',' + exc + ')');
  ...
  public synchronized void replyAborted(int replyId) {
    if (JoramTracing.dbgClient.isLoggable(BasicLevel.DEBUG))
      JoramTracing.dbgClient.log(
        BasicLevel.DEBUG, "Requestor.replyAborted(" + 
        replyId + ')');
    if (status == Status.RUN &&
        replyId == requestId) {
      reply = null;
  ...
  public synchronized void abortRequest() {
    if (JoramTracing.dbgClient.isLoggable(BasicLevel.DEBUG))
      JoramTracing.dbgClient.log(
        BasicLevel.DEBUG, 
        "Requestor[" + Status.toString(status) + 
        ',' + requestId + "].abortRequest()");
    if (status == Status.RUN && requestId > 0) {
      mtpx.abortRequest(requestId);
      setStatus(Status.DONE);
      if (JoramTracing.dbgClient.isLoggable(BasicLevel.DEBUG))
        JoramTracing.dbgClient.log(
          BasicLevel.DEBUG, " -> notify requestor");
      notify();
    }
  ...
  public synchronized void close() {
    if (JoramTracing.dbgClient.isLoggable(BasicLevel.DEBUG))
      JoramTracing.dbgClient.log(
        BasicLevel.DEBUG, "Requestor.close()");
    if (status != Status.CLOSE) {
      abortRequest();
