 ...
 */
public class DistributedJndiServer extends JndiServer 
  ...
  public static void init(String args, boolean firstTime) throws Exception {
    ...
    if (! firstTime) return;
      ...
      DistributedJndiServer a = 
        new DistributedJndiServer(serverIds);
      a.deploy();
    }
  }
  ...
  public DistributedJndiServer(short[] serverIds) {
    super();
    this.serverIds = serverIds;
    if (serverIds.length > 0) {
      rootOwnerId = 
        getDefault(serverIds[0]);
    } else {
      rootOwnerId = getId();
    }
  }
  ...
  public void agentInitialize(boolean firstTime) throws Exception {
    ...
    if (servers == null) {
      ...
      for (int i = 0; i < serverIds.length; i++) {        
        AgentId aid = getDefault(serverIds[i]);
  ...
  /**
   * Notification entry point (extends the <code>JndiServer</code>
   * entry point).
   */
  public void react(AgentId from, Notification not) throws Exception {
    if (Trace.logger.isLoggable(BasicLevel.DEBUG))
      Trace.logger.log(BasicLevel.DEBUG, 
                       "\n\nDistributedJndiServer.react(" + 
                       from + ',' + not + ')');
    if (not instanceof JndiUpdateNot) {
      doReact(from, (JndiUpdateNot)not);
    } else if (not instanceof JndiReplyNot) {
      doReact(from, (JndiReplyNot)not);
    } else if (not instanceof JndiScriptReplyNot) {
      doReact(from, (JndiScriptReplyNot)not);
    } else if (not instanceof InitJndiServerNot) {
      doReact(from, (InitJndiServerNot)not);
    } else if (not instanceof SyncRequestNot) {
      doReact(from, (SyncRequestNot)not);
    } else if (not instanceof SyncReplyNot) {
      doReact(from, (SyncReplyNot)not);
    } else {
   ...
   */
  private void doReact(AgentId from, JndiUpdateNot not) throws Exception {
...
//   }
  ...
  protected JndiReply invokeOwner(AgentId owner,
    ...
    if (Trace.logger.isLoggable(BasicLevel.DEBUG))
      Trace.logger.log(BasicLevel.DEBUG, 
                       "DistributedJndiServer.invokeOwner(" +
                       owner + ',' + reqCtx + ')');
  ...
  private void doReact(AgentId from, JndiScriptReplyNot not) 
    throws Exception {
    onReply(from, not.getReplies()[0]);
  }

  private void doReact(AgentId from, JndiReplyNot not) 
    throws Exception {
  ...
  private void doReact(AgentId from, 
    ...
    if (newServers.size() > 0) {
      ...
      NamingContextInfo[] localContexts = 
        getServerImpl().getNamingContexts(getId());
  ...
  protected JndiReply onMissingContext(MissingContextException mce,
    ...
    if (Trace.logger.isLoggable(BasicLevel.DEBUG))
      Trace.logger.log(BasicLevel.DEBUG, 
                       "DistributedJndiServer.onMissingContext(" +
                       mce + ',' + reqCtx + ')');
    RequestContextList ctxList = 
  ...
  protected JndiReply onMissingRecord(MissingRecordException mre,
    ...
    if (Trace.logger.isLoggable(BasicLevel.DEBUG))
      Trace.logger.log(BasicLevel.DEBUG, 
                       "DistributedJndiServer.onMissingRecord(" +
                       mre + ',' + reqCtx + ')');
  ...
  private void synchronizeRequest(AgentId owner, 
    ...
    if (Trace.logger.isLoggable(BasicLevel.DEBUG))
      Trace.logger.log(
        BasicLevel.DEBUG, 
        "DistributedJndiServer.synchronizeRequest(" +
        owner + ',' + reqCtx + ')');
  ...
  private void doReact(AgentId from, SyncRequestNot not) {
    sendTo(from, new SyncReplyNot());
  }

  private void doReact(AgentId from, SyncReplyNot not) {
    RequestContextList ctxList = 
