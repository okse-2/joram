 ...
 *  - Wait for the ReconnectionDaemon success before stopping the server so there is
 *  a connection close and a call to JMSModule.onException.
 *  - sends a message on Joram server
 *  - restarts the foreign server
 *  - receives the message on foreign destination
 */
public class BridgeTestX2 extends TestCase {

  public static void main(String[] args) {
    new BridgeTestX2().run();
  }
  ...
  public void run() {
    try {
      ...
      // Wait for the ReconnectionDaemon success, then stop the server so there is
      // a connection close and a call to JMSModule.onException.
      Thread.sleep(5000);
