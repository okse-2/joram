 ...
 */
public class ClearPendingMessageActionImpl extends
    ActionImpl<ClearPendingMessageResponse, ClearPendingMessageAction, RPCServiceImpl> {

  @Override
  public ClearPendingMessageResponse execute(RPCServiceImpl cache,
      ClearPendingMessageAction clearPendingMessageAction) {
    boolean result = cache.cleanPendingMessage(clearPendingMessageAction.getQueueName());

    String info = "";

    if (!result) {
      info = BaseRPCServiceUtils.getString("Error while clearing pending messages : Queue \""
          + clearPendingMessageAction.getQueueName() + "\" not found.");
    }
