...
import fr.dyade.aaa.common.Pool;
   ...
   * In the class <b>MBeanServerConnectionDelegate<b> is implemented all the methods that can be call'd by the administration tool (JConsole)
   * 
  ...
  public class  MBeanServerConnectionDelegate  implements MBeanServerConnection {
      ...
      public  ObjectInstance createMBean(String className,ObjectName name,ObjectName loaderName) throws ReflectionException,InstanceAlreadyExistsException,MBeanRegistrationException,MBeanException,NotCompliantMBeanException,InstanceNotFoundException,IOException{
    	  ...
    	  System.out.println("Appel a la methode ObjectInstance createMBean(String className,ObjectName name,ObjectName loaderName)");
     ...
     /**
      * <b>addNotificationListener</b> When a requestor uses this method, an object is created <i>objectAddNotificationListenerStored</i>, with the same  parameters call , then this object is stored in <i>the hashTableNotificationListener</i> HashMap that will allow us later  find the object corresponding to the  <i>addNotificationListener</i> call,  so we can  remove the right object when a <i>removeNotificationListener</i> is made..
      * 
      * @param ObjectName 
     ...
     /**
      * <b>removeNotificationListener</b> This method is called to remove a listener registered after call to the <i>addNotificationListener</i>, to remove the  good listener we consult the  <i>hashTableNotificationListener</i> HashMap to find the right objetLister registered, once the object  found, we recover  the <i>key</i> which is passed in parameter like <i>handback</i>, of the <i>RemoveNotificationListeber</i> object which will be send, so that we can on the other side (server side),  find the right listener from receiving the key, and delete  it by using the method JMX <i>removeNotificationListener</i>.
      * 
      * @param ObjectName 
      * @param ObjectName 
      * @throws JMSException
    	      ...
    	      if(pairKeyListener.getValue().equals(objectAddNotificationListenerStored)){
    	    	  keyRestored = pairKeyListener.getKey();
    			   ...
    			   System.out.println("------------->    keyRestored de removeNotificationListener(ObjectName name,NotificationListener listener)  : "+keyRestored);
    		    	  ...
    		    	  System.out.println("Contenu de la hashTableNotificationListener : "+hashTableNotificationListener);
    		    	  System.out.println("***----> l'objet objectAddNotificationListenerStored"+hashTableNotificationListener.get(keyRestored) +" a ete supprimé de la hashTableNotificationListener ");
    		    	  RemoveNotificationListener2 objectRemoveNotificationListener2 = new RemoveNotificationListener2(name,listener,keyRestored);
    		    	  try {
    		    		  Requestor requestor;
    		    		  requestor = (Requestor) poolRequestors.allocRequestor();
    		    	   	  System.out.println("********* * * * Jai pris le requestor numero : "+requestor.toString());
     ...
     */
     ...
     public  void removeNotificationListener(ObjectName name,ObjectName listener,NotificationFilter filter,Object handback) throws InstanceNotFoundException,ListenerNotFoundException,IOException{
    	  ...
    	  System.out.println("**************************Appel a la methode removeNotificationListener(ObjectName name,ObjectName listener,NotificationFilter filter,Object handback)" );
    	  Object keyRestored = null;
    	  value--;
    	  AddNotificationListenerStored objectAddNotificationListenerStored = new AddNotificationListenerStored(name, listener, filter, handback); 
    	  Iterator<Map.Entry<Integer,AddNotificationListenerStored>> it = hashTableNotificationListener.entrySet().iterator();
    	      ...
    	      if(pairKeyListener.getValue().equals(objectAddNotificationListenerStored)){
    	    	  keyRestored = pairKeyListener.getKey();
    			   ...
    			   System.out.println("------------->    keyRestored de removeNotificationListener(ObjectName name,NotificationListener listener)  : "+keyRestored);
    		    	  ...
    		    	  System.out.println("Contenu de la hashTableNotificationListener : "+hashTableNotificationListener);
    		    	  System.out.println("***----> l'objet objectAddNotificationListenerStored"+hashTableNotificationListener.get(keyRestored) +" a ete supprimé de la hashTableNotificationListener ");
    		    	  RemoveNotificationListener3 objectRemoveNotificationListener3 = new RemoveNotificationListener3(name,listener,filter,keyRestored);
    		    	  try {
    		    		  Requestor requestor;
    		    		  requestor = (Requestor) poolRequestors.allocRequestor();
    		    	   	  System.out.println("********* * * * Jai pris le requestor numero : "+requestor.toString());
    		    		  ...
    		    		  break; //The MBean must have a listener that exactly matches the given listener, filter, and handback parameters. If there is more than one such listener, only one is removed.
    				} catch (JMSException e) {
    					// TODO Auto-generated catch block
    					e.printStackTrace();
    				}
    	      ...
    	      if(pairKeyListener.getValue().equals(objectAddNotificationListenerStored)){
    	    	  keyRestored = pairKeyListener.getKey();
    			   ...
    			   System.out.println("------------->    keyRestored de removeNotificationListener(ObjectName name,NotificationListener listener)  : "+keyRestored);
    		    	  ...
    		    	  RemoveNotificationListener objectRemoveNotificationListener = new RemoveNotificationListener(name,keyRestored);
    		    	  try {
    		    		  Requestor requestor;
    		    		  requestor = (Requestor) poolRequestors.allocRequestor();
    		    	   	  System.out.println("********* * * * Jai pris le requestor numero : "+requestor.toString());
     ...
     public  void removeNotificationListener(ObjectName name,NotificationListener listener,NotificationFilter filter,Object handback) throws InstanceNotFoundException,ListenerNotFoundException,IOException{
    	  ...
    	  while (it.hasNext()) {
    	      ...
    	      if(pairKeyListener.getValue().equals(objectAddNotificationListenerStored)){
    			  ...
    			  RemoveNotificationListener1 removeNotificationListener1 = new RemoveNotificationListener1(name, filter, keyRestored);
    			  try{
    				  Requestor requestor;
    				  requestor = (Requestor) poolRequestors.allocRequestor();
    				  System.out.println("********* * * * Jai pris le requestor numero : "+requestor.toString());
