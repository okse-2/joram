...
/**
 * A <b>Requestor </b> a requestor allows to do one or many JMS requetes to the server connector , through the server JORAM.
...
/**
 *  <ul>the <b>createEntitiesOfAdministration(Connection connection)</b> is called during initialization of a requestor, she allows to create the logical entities of directors from a common connection to all the requestor creates.</ul>
 *  <ul> The logical entities are :</ul>
 *  <ul><li> Two Sessions.</ul></li>
 *  <ul><li> Two Destination-type "Queue" (a for request-response and the other  for notifications).</ul></li>
 *  <ul><li> A producer, consumer.</ul></li>
 * 
 * @param Connection  <ul><li>The connection which is common for all requestor.</ul></li> 
 * @throws JMSException
 */
...
/**
 * <b>onMessage</b> in this method  is implemented the listener of the QNotification destination for receiving the notifications issued by the registered MBean in the MBeanServer.
 * 
 * @param Message       <ul>the message parameter to the method represents the message received .</ul>  
 * @throws JMSException
 */
...
public void onMessage(Message message){
	...
	try {
	 ...
	 ((NotificationListener)objectAddNotificationListenerStored.listener).handleNotification(notificationAndKey.notification, objectAddNotificationListenerStored.handback);
...
/**
 * <b>doRequete</b> is the method that is called by a requestor for send a message of type ObjectMesage.
 * 
 * @param Object       <ul>The object that is passed as a parameter is the object which will be send to the destination of server connector .</ul>   
 * @throws JMSException
 */
...
/**
 * <b>initializationPropertiesOfMessageSending</b> this method is invoked by a requester before sending a message to initialize the properties of  the message to send.
 * 
 * @param	ObjectMessage  <ul> represents the message that will be modified by changing his properties.</ul>
 * @param 	String		   <ul> represents the name of the key of the property of the message.</ul>
 * @param 	String		   <ul> represents the value of the key in the properties of the message which is the name of the server connector. </ul>
 * @param 	Destination    <ul> represents the destination of the sender (requestor).</ul>
 * @return 	ObjectMessage   
 * @throws 	JMSException
 */


public ObjectMessage initializationPropertiesOfMessageSending(ObjectMessage message,String key,String connectorName,Destination ReplyTo){
	try {
...
/**
 * <b>doReceive</b> is the method that is called by a requestor to retrieve the response of the request sent by the  doRequete method.
 * 
 * @param void       
 * @return ObjectMessage   <ul><li>This method returns the received message (Object Message).</ul></li>
 * @throws JMSException
 */
...
/**
 * <b>subscribeToNotifications</b> is the method that is called by a requestor to subscribe to notifications obligations issued by the registered MBean in the MBeanServer
 * 
 * @param Object        <ul>The object that is passed as a parameter is the object which will be send to the destination of server connector .</ul>
 * @throws JMSException
 */
