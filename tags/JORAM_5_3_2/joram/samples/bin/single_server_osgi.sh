...
cd $RUN_DIR; exec "${JAVA_HOME}"/bin/java  -Dosgi.shell.telnet.port=16000 -Dfelix.config.properties=file:config.properties -Dfelix.cache.rootdir=./s0 -Dcom.sun.management.jmxremote -DMXServer=com.scalagent.jmx.JMXServer -Dfr.dyade.aaa.agent.AgentServer.id=0 -jar $JORAM_BIN/felix.jar
