...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    for(int l=0;l<nbDest;l++){
      char q[24];
      sprintf(q,"%s%i","queue",l);
      queue[l]=admin->createQueue(q);
      admin->setFreeReading(queue[l]);
      admin->setFreeWriting(queue[l]);
    }
    ...
    MessageProducer* prod1 = sess1->createProducer(NULL);
    MessageConsumer* cons1[nbDest];
    for(int l=0;l<nbDest;l++){
      cons1[l]= sess1->createConsumer(queue[l]);
    }
    ...
    long travel=0;
    if (prod == TRUE) 
      sleep(2); // sleep producer during consumer attach to queue
    ...
    if (cons == TRUE) {
      printf("consumer : %ld msg/s\n",(long)((1000*nbMessage*nbDest)/total));
      printf("mean travel : %ld ms\n",(long)(travel/(nbMessage*nbDest)));
      char *temp=new char[300];
       char *temp2=new char[256];
       strcpy(temp,"consumer : ");
       sprintf(temp2,"%ld",(long)((1000*nbMessage*nbDest)/total));
       strcat(temp,temp2);
       strcat(temp," msg/s\n mean travel : ");
       sprintf(temp2,"%ld",(long)(travel/(nbMessage*nbDest)));
       strcat(temp,temp2);
       strcat(temp," ms");
