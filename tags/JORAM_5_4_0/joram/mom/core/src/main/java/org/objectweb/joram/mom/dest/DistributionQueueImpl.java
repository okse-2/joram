...
/**
 * The {@link DistributionQueueImpl} class implements the MOM distribution queue
 * behavior, delivering messages via the {@link DistributionModule}.
 */
public class DistributionQueueImpl extends QueueImpl {

  public static Logger logger = Debug.getLogger(DistributionQueueImpl.class.getName());
  ...
  /**
   * Constructs a {@link DistributionQueueImpl} instance.
   * 
   * @param adminId
  ...
  public DistributionQueueImpl(AgentId adminId, Properties properties) throws RequestException {
    ...
    if (logger.isLoggable(BasicLevel.DEBUG)) {
      logger.log(BasicLevel.DEBUG, "DistributionQueueImpl.<init> prop = " + properties);
    }
    ...
    try {
      ...
      logger.log(BasicLevel.ERROR, "DistributionQueueImpl: error with distribution class.", exc);
  ...
  public void initialize(boolean firstTime) {
    ...
    if (distributionModule == null) {
      try {
        ...
        logger.log(BasicLevel.ERROR, "DistributionQueueImpl.initialize prop = " + properties, exc);
      }
    }
  }
  ...
  public ClientMessages preProcess(AgentId from, ClientMessages cm) {
    if (logger.isLoggable(BasicLevel.DEBUG)) {
      logger.log(BasicLevel.DEBUG, "DistributionQueueImpl. preProcess(" + from + ", " + cm + ')');
    }
    return distributionModule.processMessages(cm);
  }

  public String toString() {
    return "DistributionQueueImpl:" + getId().toString();
  }
