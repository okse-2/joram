...
/**
 * Check exceptions on message.send() when topic has been deleted. Two different
 * behaviors are expected if the topic is local and synchronous or
 * remote/asynchronous.
 */
public class Test11 extends BaseTest {
  ...
  public void run() {
    try {
      ...
      ExcList11 receiverExcListener = new ExcList11("Receiver");
      ...
      MsgList11 msgListener = new MsgList11();
      cons.setMessageListener(msgListener);
      cnx1.start();

      Connection cnx2 = cf.createConnection();
      ExcList11 senderExcListener = new ExcList11("Sender");
      ...
      // If remote, the exception listener should receive the exception.
      Thread.sleep(1000);
