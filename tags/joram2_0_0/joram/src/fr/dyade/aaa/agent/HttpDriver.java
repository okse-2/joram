	...
	if(Name.equals("DEBUG_TITLE")){
	  ...
	  case 2 : // list of Servers requested
	  {
		...
		if(list.indexOf("fr.dyade.aaa.ip.HttpProxy")==-1) // this server launched the HttpProxy service 
		  ...
		  int port=Integer.parseInt(AgentServer.getServiceArgs(sid,"fr.dyade.aaa.ip.HttpProxy"));
		  String hostname = AgentServer.getHostname(sid);
...
// 					AgentServer.qin.size()+"</li>" +
// 					"<br/><li>   Numbers of messages in the queue out : " +
// 					AgentServer.qout.size()+"</li></ul>");
	    typeList = 0 ;
	    break ;
	  }
...
// 	    parser.setValidation(true);
	...
	proto ="http";
	localhost=AgentServer.getHostname(AgentServer.getServerId());
	portServer=AgentServer.getServiceArgs(AgentServer.getServerId(),"fr.dyade.aaa.ip.HttpProxy");
	path="" ;
	text="lien" ;
	...
	if(s.equals("/")){ // first page
	  ...
	  typeList = 3 ;
	  subtitle.addElement("Queues for the server "+AgentServer.getServerId());
	  print(doc);
  ...
  Vector listNetworkServer() {
    ServerDesc list[] = AgentServer.servers;
    Vector servers = new Vector();
    for (int i=0; i<AgentServer.servers.length; i++ ){
      if (! AgentServer.servers[i].isTransient)
	servers.addElement(AgentServer.servers[i].toString());
    }
    return servers;
  }
