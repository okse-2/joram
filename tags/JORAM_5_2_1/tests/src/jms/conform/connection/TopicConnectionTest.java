...
public class TopicConnectionTest extends PubSubTestCase {
   ...
   * connection creation and before any other action on the connection.
  ...
  /**
   * Test that if another connection with the same clientID is already running
   * when <code>setClientID</code> is called, the JMS provider should detect
   * the duplicate ID and throw an <code>InvalidClientIDException</code>
   * <em>This test is relevant only if the ID is set by the JMS client</em>
   * see JMS javadoc
   * http://java.sun.com/j2ee/sdk_1.3/techdocs/api/javax/jms/Connection.html#setClientID(java.lang.String)
   */
