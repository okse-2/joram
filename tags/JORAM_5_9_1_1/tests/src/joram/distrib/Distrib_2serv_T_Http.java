 ...
 * server 0. Tests with big messages (500 ko).
 */
public class Distrib_2serv_T_Http extends TestCase {
  
  static int NbRound = 10;
  static int NbMsgPerRound = 1;
  static int MsgSize = 500 * 1024;
      ...
      try {
        ...
        for (int i = 0; i < NbRound; i++) {
          for (int j = 0; j < NbMsgPerRound; j++) {
            ...
            System.out.println("send message -> " + (System.currentTimeMillis() - start) / 1000);
            producer.send(msg);
            System.out.println("msg sent -> " + (System.currentTimeMillis() - start) / 1000);
          }
          listener.fxCtrl(i);
        }
